...
config = ProtDiffusionTrainingConfig(
    num_epochs=50, # the number of epochs to train for
    batch_size=64,
    mega_batch=240,
    ...
    output_dir=os.path.join("output","ProtDiffusion-IPR036736_sinusoidal"),  # the model name locally and on the HF Hub
...
transformer = DiTTransformer1DModel(
    ...
    norm_eps = 1e-5,
    pos_embed_type = "sinusoidal", # None
    num_positional_embeddings = config.max_len // config.pad_to_multiple_of,
    use_rope_embed = False, # RoPE https://github.com/lucidrains/rotary-embedding-torch
)
...
Trainer = ProtDiffusionTrainer(transformer=transformer,
                               ...
                               eval_seq_len = [2048, 2048, 512, 512], # the sequence lengths to evaluate on
