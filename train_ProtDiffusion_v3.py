...
config = ProtDiffusionTrainingConfig(
    ...
    output_dir=os.path.join("output","ProtDiffusion-IPR036736_RoPE_v3.2.2"),  # the model name locally and on the HF Hub
...
vae = AutoencoderKL1D.from_pretrained('/work3/s204514/protein-VAE-UniRef50_v24.12_latent-4_conv_transpose/pretrained/EMA')
...
train_dataset = train_dataset.shuffle(config.seed + 24) # restart at checkpoint 23
...
Trainer = ProtDiffusionTrainer(transformer=transformer,
                               ...
                               eval_guidance_scale = 1.0, # the scale of the guidance for the diffusion
                               eval_num_inference_steps = 100, # the number of inference steps for the diffusion
)

# %%
if __name__ == '__main__':
    Trainer.train(from_checkpoint='/zhome/fb/0/155603/output/ProtDiffusion-IPR036736_RoPE_v3/checkpoints/checkpoint_23')